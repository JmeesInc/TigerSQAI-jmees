...
- ローカル: `data/`（symlink → `/mnt/data/data4/shared/miccai/EndoVis2026/tiger`、**読み取り専用扱い**）。
...
- **段階配布**: Part 1（2026/5末, **今ローカルにある 10 ケース=140枚**）→ Part 2（**2026/7月, 残り train ケース**）。test 10 ケースは非公開（Docker で採点）。
- 今ローカルにある Part 1: `center_1_case_6` 〜 `center_1_case_15`（10 ケース）。`masks_fine/` `masks_coarse/` 各 140 枚（画像と同名）。
- 解像度: **1920×1080 が 126 枚 / 1280×720 が 14 枚**（混在）。Part 2 で別の解像度・センターが来る可能性に留意。
...
## 6. タイムライン（**Synapse で確定** — `639482`。すべて AoE / UTC−12）

| 日程 | 内容 |
|------|------|
| 2026/04/10 – 08/31 | 登録期間 |
| 2026/05 末 | 訓練データ Part 1 公開（←現在） |
| **2026/07** | **訓練データ Part 2 公開** |
| 2026/09/01 | 評価開始 |
| **2026/09/15** | **提出締切** |
| 2026/10/08 – 12 | Challenge Day（EndoVis 2026 で発表） |

- 締切まで残り約3ヶ月（2026-06-17 時点）。**遅延提出は不可**（技術的問題のみ要相談）。
- 単一の評価フェーズ（Sep 1 開始、Sep 15 締切）。別個の validation phase は明記なし。
...
- [ ] 商用ライセンスモデルの可否（明記なし）
- [ ] Docker I/O 契約の詳細（入力パス/出力パス/実行時間・GPU 制限）→ `639935` Docker Instructions と Docker Template `639939` を読む（実装直前に）
