...
class SegModule(pl.LightningModule):
    def __init__(self, cfg: dict, class_weights: dict):
        ...
        # DINOv3-7B (凍結) を教師とした特徴空間蒸留。**推論時は使わない**
        # (checkpoint から除外するので提出用 ckpt は expA06 と完全同形式)
        self.w_distill = float(lc.get("distill_weight", 0.0))
        if self.w_distill > 0:
            import timm
            vit = timm.create_model("vit_7b_patch16_dinov3.lvd1689m", pretrained=True,
                                    num_classes=0, img_size=(cfg["data"]["img_h"], cfg["data"]["img_w"]))
            vit.eval().half()
            for p_ in vit.parameters():
                p_.requires_grad_(False)
            self.teacher = vit
            self.t_grid = (cfg["data"]["img_h"] // 16, cfg["data"]["img_w"] // 16)
            self.proj = nn.Conv2d(self.model.encoder.out_channels[4], vit.embed_dim, 1)
            log.info("distill: DINOv3-7B teacher (frozen fp16), proj %d->%d, w=%.3f",
                     self.model.encoder.out_channels[4], vit.embed_dim, self.w_distill)
    ...
    def train(self, mode: bool = True):  # 凍結 teacher は常に eval
        super().train(mode)
        if getattr(self, "w_distill", 0) > 0:
            self.teacher.eval()
        return self

    def training_step(self, batch, _):
        need_feats = self.w_distill > 0
        if need_feats:
            lf_logit, lc_logit, s_feat = self.model(batch["image"], return_feats=True)
        else:
        ...
        if need_feats:
            with torch.no_grad():
                t = self.teacher.forward_features(batch["image"].half())
                t = t[:, self.teacher.num_prefix_tokens:]
                b, n, d = t.shape
                t = t.transpose(1, 2).reshape(b, d, *self.t_grid).float()
            sp = self.proj(s_feat.float())
            loss_dist = 1.0 - torch.nn.functional.cosine_similarity(sp, t, dim=1).mean()
            loss = loss + self.w_distill * loss_dist
            logs["train/loss_distill"] = loss_dist
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        for k in [k for k in sd if k.startswith("teacher.") or k.startswith("proj.")]:
            del sd[k]
