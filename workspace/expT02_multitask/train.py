...
class SegModule(pl.LightningModule):
    ...
    def training_step(self, batch, _):
        lf_logit, lc_logit, lt3 = self.model(batch["image"], batch["station_onehot"])
        ...
        w_t3 = float(self.cfg["loss"].get("task3_weight", 0.0))
        if w_t3 > 0:
            bce = torch.nn.functional.binary_cross_entropy_with_logits(
                lt3.float(), batch["t3_target"], reduction="none").mean(1)
            m = batch["t3_mask"]
            loss_t3 = (bce * m).sum() / m.sum().clamp(min=1)
            loss = loss + w_t3 * loss_t3
            logs["train/loss_t3"] = loss_t3
    ...
    def validation_step(self, batch, _):
        lf_logit, lc_logit, _ = self.model(batch["image"], batch["station_onehot"])
