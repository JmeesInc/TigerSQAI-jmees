...
class TigerDataset(Dataset):
    def __init__(self, df: pd.DataFrame, repo: Path, cfg: dict, train: bool):
        ...
        self.tool_dir = repo / "workspace/data_proc/tool_masks_1024"
    ...
    def __getitem__(self, i: int):
        ...
        tool = cv2.imread(str(self.tool_dir / row.filename), cv2.IMREAD_GRAYSCALE)
        out = self.tf(image=img, masks=[fine, coarse, tool])
        return {
            "image": out["image"],
            ...
            # 器具マスク (1=器具)。幾何 aug 後の座標系で画像・ラベルと整合する
            "tool": torch.from_numpy(np.ascontiguousarray(out["masks"][2])).float() / 255.0,
        }
