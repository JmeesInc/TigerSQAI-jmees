...
FINE_INSTRUMENT_ID, COARSE_INSTRUMENT_ID = 1, 12


def load_tool_model(device: str):
    """器具専用モデル (STIR convnext-unet)。学習時に器具画素を除外したぶん、
    推論では Instrument をこのモデルが担当する。"""
    ...
    st = torch.load(STIR_CKPT, map_location="cpu", weights_only=False)
...
def predict_fold(cfg: dict, fold: int, out1: Path, out2: Path, device: str,
    ...
    for _, row in val_df.iterrows():
        ...
        with torch.autocast("cuda", dtype=torch.float16):
            lf, lc = model(x)
        # 器具マスク (元解像度)
        xt = cv2.cvtColor(cv2.resize(bgr, (512, 512), interpolation=cv2.INTER_AREA), cv2.COLOR_BGR2RGB)
        xt = (xt.astype(np.float32) / 255.0 - np.array(IMAGENET_MEAN, np.float32)) / np.array(IMAGENET_STD, np.float32)
        xt = torch.from_numpy(xt.transpose(2, 0, 1))[None].half().to(device)
        with torch.autocast("cuda", dtype=torch.float16):
            tp = tool(xt)
        tmask = (F.interpolate(tp.float(), size=(oh, ow), mode="bilinear",
                               align_corners=False)[0, 0] > 0.5).cpu().numpy()

        for logits, id2rgb, out_dir, instr_id in [
            (lf, id2rgb_fine, out1, FINE_INSTRUMENT_ID), (lc, id2rgb_coarse, out2, COARSE_INSTRUMENT_ID)
        ]:
            up = F.interpolate(logits.float(), size=(oh, ow), mode="bilinear", align_corners=False)
            ids = up.argmax(1)[0].cpu().numpy().astype(np.uint8)
            ids[tmask] = instr_id      # 器具は専用モデルが担当
