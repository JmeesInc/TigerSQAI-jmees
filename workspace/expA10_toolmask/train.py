...
class MaskedDiceLoss(torch.nn.Module):
    """ピクセルマスク付き weighted soft Dice。

    keep=0 の画素は予測・GT の両方から除外され、その画素は一切 loss に寄与しない。
    MONAI の DiceLoss はピクセル単位マスクを取れないため自前実装。
    keep が全 1 のとき MONAI DiceLoss(softmax, to_onehot_y, include_background, weight) と一致する。
    """

    def __init__(self, class_weights: list[int], n_classes: int, eps: float = 1e-5,
                 softmax: bool = True):
        super().__init__()
    ...
    def forward(self, logits, target, keep):
        # logits B,C,H,W (softmax=False なら確率) / target B,H,W (long) / keep B,1,H,W
        p = (logits.float().softmax(1) if self.softmax else logits.float()) * keep
        t = torch.nn.functional.one_hot(target, self.n).permute(0, 3, 1, 2).float() * keep
        inter = (p * t).sum(dim=(2, 3))
        denom = p.sum(dim=(2, 3)) + t.sum(dim=(2, 3))
        dice = (2 * inter + self.eps) / (denom + self.eps)
        per_class = 1.0 - dice                      # B,C
        return (per_class * self.w).sum(1).div(self.w.sum()).mean()
...
class SegModule(pl.LightningModule):
    def __init__(self, cfg: dict, class_weights: dict):
        ...
        lc = cfg["loss"]
        # 器具画素を除外する masked Dice に差し替え (tool model のマスクを使用)
        self.tool_mask_mode = lc.get("tool_mask", "off")   # off | anat_only
        if self.tool_mask_mode == "anat_only":
            self.loss_fine = MaskedDiceLoss(class_weights["fine"], len(class_weights["fine"]))
            self.loss_coarse = MaskedDiceLoss(class_weights["coarse"], len(class_weights["coarse"]))
            log.info("tool-masked Dice: 器具画素を loss から除外 (Instrument は推論時に tool model が担当)")
        else:
        ...
        if self.w_f2c > 0:
            ...
            if self.tool_mask_mode == "anat_only":
                self.loss_f2c = MaskedDiceLoss(class_weights["coarse"], len(class_weights["coarse"]),
                                               softmax=False)
            else:
    ...
    def training_step(self, batch, _):
        ...
        if self.tool_mask_mode == "anat_only":
            keep = (1.0 - batch["tool"]).unsqueeze(1)     # B,1,H,W  器具画素を 0 に
            loss_f = self.loss_fine(lf_logit, batch["fine"], keep)
            loss_c = self.loss_coarse(lc_logit, batch["coarse"], keep)
        else:
        ...
        if self.w_f2c > 0:
            ...
            if self.tool_mask_mode == "anat_only":
                loss_f2c = self.loss_f2c(probs_c_from_f, batch["coarse"], keep)
            else:
