...
class SegModule(pl.LightningModule):
    def __init__(self, cfg: dict, class_weights: dict):
        ...
        # 凍結 ViT (13.4GB) は checkpoint に保存しない (resume 時は HF cache から再ロード)
        ...
        # fine logits を merged_id 単位で確率合算 -> coarse GT に対する階層一貫性 loss (expA06 と同一)
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        for k in [k for k in sd if ".encoder.vit." in k]:
            del sd[k]
