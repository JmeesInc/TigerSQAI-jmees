...
class SegModule(pl.LightningModule):
    def __init__(self, cfg: dict, class_weights: dict):
        ...
        lc = cfg["loss"]
        # 損失側のみクラス重みを weight_power 乗する (3/2/1 -> 9/4/1 等)。
        # 監視メトリクス (wf/wc) と公式評価は従来重みのまま
        pw = float(lc.get("weight_power", 1.0))
        lw_fine = [w ** pw for w in class_weights["fine"]]
        lw_coarse = [w ** pw for w in class_weights["coarse"]]
        self.loss_fine = build_loss(lc["name"], lw_fine)
        self.loss_coarse = build_loss(lc["name"], lw_coarse)
        ...
        if self.w_f2c > 0:
            ...
            self.loss_f2c = DiceLoss(
                ...
                weight=torch.tensor(lw_coarse, dtype=torch.float32),
            )
