...
def main() -> None:
    ...
    ap.add_argument("--init-from", default=None,
                    help="学習済み ckpt から model 重みのみを初期化する (合成事前学習 -> 実データ fine-tune)")
    args = ap.parse_args()
    ...
    if args.init_from:
        # Weights only: optimiser/scheduler/epoch state must not carry over from pretraining.
        sd = torch.load(args.init_from, map_location="cpu", weights_only=False)["state_dict"]
        missing, unexpected = module.load_state_dict(sd, strict=False)
        log.info("init from %s | missing %d unexpected %d", args.init_from, len(missing), len(unexpected))
        assert not [k for k in missing if k.startswith("model.")], f"model weights missing: {missing[:5]}"
