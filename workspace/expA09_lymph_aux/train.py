...
class SegModule(pl.LightningModule):
    def __init__(self, cfg: dict, class_weights: dict):
        ...
        # Task3 補助: fine の Lymph_Node チャネル (id=19) の確率マップを空間集約し、
        # station one-hot と併せて 14 ステーション可視性を予測する。
        # **Task3 のラベルが Lymph_Node セグメンテーションに直接勾配を送る**のが狙い
        # (expT02 の全 14 クラス MTL は encoder を乱して失敗 → 経路を Lymph_Node に絞る)
        self.w_lymph = float(lc.get("lymph_aux_weight", 0.0))
        if self.w_lymph > 0:
            self.lymph_id = 19
            self.lymph_head = nn.Sequential(
                nn.Linear(4 + 15, 64), nn.ReLU(inplace=True), nn.Linear(64, 14)
            )
            log.info("lymph aux: Lymph_Node(ch%d) prob -> pooled(4) + onehot(15) -> 14, w=%.3f",
                     self.lymph_id, self.w_lymph)
    ...
    def training_step(self, batch, _):
        ...
        if self.w_lymph > 0:
            pl = lf_logit.softmax(1)[:, self.lymph_id]           # B,H,W  Lymph_Node 確率
            b = pl.shape[0]
            half = pl.shape[2] // 2
            stats = torch.stack([                                 # 空間集約 4 特徴
                pl.mean(dim=(1, 2)),                              # 全体量
                pl.amax(dim=(1, 2)),                              # 最大確信度
                pl[:, :, :half].mean(dim=(1, 2)),                 # 左右で分ける (L/R station 用)
                pl[:, :, half:].mean(dim=(1, 2)),
            ], dim=1)
            lt3 = self.lymph_head(torch.cat([stats, batch["station_onehot"]], dim=1))
            bce = torch.nn.functional.binary_cross_entropy_with_logits(
                lt3.float(), batch["t3_target"], reduction="none").mean(1)
            m = batch["t3_mask"]
            loss_lymph = (bce * m).sum() / m.sum().clamp(min=1)
            loss = loss + self.w_lymph * loss_lymph
            logs["train/loss_lymph"] = loss_lymph
    ...
    def on_validation_epoch_end(self):
        ...
        if getattr(self, "w_lymph", 0) > 0 and score > getattr(self, "_best_lymph", -1.0):
            self._best_lymph = score
            torch.save(self.lymph_head.state_dict(),
                       Path(self.trainer.default_root_dir) / "lymph_head_best.pt")
        self.history.append(
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        for k in [k for k in sd if k.startswith("lymph_head.")]:
            del sd[k]
