"""expA23: あるレシピの **水平反転 TTA 付き OOF softmax 確率**を half 解像度 fp16 で保存する.

(save_probs.py の TTA 版。元画像と左右反転の softmax を平均する。出力は results/<name>_tta/probs/)
...
def main() -> None:
    ...
    out_root = REPO / cfg["paths"]["results_root"] / f"{name}_tta"
    out_f = out_root / "probs" / "fine"
    out_c = out_root / "probs" / "coarse"
    ...
    for fold in args.folds:
        ...
        for _, row in val_df.iterrows():
            ...
            with torch.autocast("cuda", dtype=torch.float16):
                lf, lc = model(x)
                lf2, lc2 = model(torch.flip(x, dims=[3]))
            for logit, logit2, out_dir in ((lf, lf2, out_f), (lc, lc2, out_c)):
                prob = 0.5 * (logit.float().softmax(1) + torch.flip(logit2.float().softmax(1), dims=[3]))
                p = F.interpolate(prob, size=(OUT_H, OUT_W),
    ...
    log.info("合計 %d 枚 -> %s", n_total, out_root / "probs")
