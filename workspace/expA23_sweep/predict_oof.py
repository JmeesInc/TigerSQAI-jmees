...
def load_model(cfg: dict, ckpt_path: Path, device: str) -> torch.nn.Module:
    """expA23 の汎用ビルダで組み、Lightning ckpt / fp16 重みのどちらでも読む。"""
    m = dict(cfg["model"])
    m["encoder_weights"] = None          # 事前学習を落としてから ckpt を被せる (DL 不要)
...
def predict_fold(cfg: dict, fold: int, out1: Path, out2: Path, device: str,
    ...
    if not ckpt.exists():                 # prune 済み run は fp16 重みだけが残る
        ckpt = fold_dir / "best_fp16.pt"
    assert ckpt.exists(), f"missing ckpt in {fold_dir}"
