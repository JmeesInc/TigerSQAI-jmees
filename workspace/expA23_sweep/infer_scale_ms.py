...
SCALES = [(512, 896), (544, 960), (576, 1024)]
# マルチスケール平均（原寸へ戻してから確率平均）
MULTI = [[(512, 896), (576, 1024)], [(512, 896), (544, 960), (576, 1024)]]
        ...
        for _, row in val.iterrows():
            ...
            for (ih, iw) in combo:
                ...
                with torch.no_grad():
                    lf, lc = net(x)
                for task, lg in (("fine", lf), ("coarse", lc)):
                    up = F.interpolate(lg.float().softmax(1), size=(oh, ow), mode="bilinear",
                                       align_corners=False)
                    probs[task] = up if probs[task] is None else probs[task] + up
            case = row.filename.rsplit(".", 1)[0].rsplit("_", 1)[0]
            for task in ("fine", "coarse"):
                lut, ids, w, n, gt_sub = T[task]
                pred = probs[task].argmax(1)[0].to(torch.uint8).cpu().numpy()
                g = decode(REPO / "data" / gt_sub / row.filename, lut)
                acc[task].setdefault(case, []).append(wdice(pred, g, ids, w, n))
        sc = {t: float(np.mean([np.mean(v) for v in acc[t].values()])) for t in acc}
        sc["mean"] = (sc["fine"] + sc["coarse"]) / 2
        tag = "+".join(f"{h}" for h, _ in combo)
        out[f"MS({tag})"] = {k: round(v, 4) for k, v in sc.items()}
        print(f"MS({tag}): fine {sc['fine']:.4f}  coarse {sc['coarse']:.4f}  平均 {sc['mean']:.4f}", flush=True)
