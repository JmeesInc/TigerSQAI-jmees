...
def predict_fold(cfg: dict, fold: int, out1: Path, out2: Path, device: str,
                 limit: int | None = None, ckpt_name: str = "best.ckpt") -> None:
...
def main() -> None:
    ...
    ap.add_argument("--ckpt-name", default="best.ckpt", help="使用する重み (best.ckpt | swa.ckpt)")
    args = ap.parse_args()

    cfg = yaml.safe_load(Path(args.config).read_text())
    if args.exp_name:
        cfg["experiment"]["name"] = args.exp_name
    suffix = "" if args.ckpt_name == "best.ckpt" else "_" + Path(args.ckpt_name).stem
    oof_dir = REPO / cfg["paths"]["results_root"] / cfg["experiment"]["name"] / f"oof{suffix}"
