"""expT01: Task3 特化ヘッド — expA06 学習済み encoder (凍結) + station one-hot + MLP.

- 入力: 1024x576 キャッシュ画像 + ファイル名 station の one-hot (14+1: na 用に15次元)
  （station は test 時もファイル名から得られる正当な入力。518/518 で「自станция可視」が成立）
...
def station_onehot(stem: str) -> np.ndarray:
    st = stem.split("case_")[1].split("_", 1)[1]  # e.g. "6L" or "na_station_1"
...
class T3Dataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return (
            ...
            torch.from_numpy(station_onehot(r.case_id)),
...
class T3Head(nn.Module):
    def __init__(self, encoder: nn.Module, feat_ch: int, finetune: bool):
        ...
        self.head = nn.Sequential(
            nn.Linear(feat_ch + len(STATIONS) + 1, 256), nn.ReLU(inplace=True),
    ...
    def forward(self, x, onehot):
        ...
        return self.head(torch.cat([g, onehot], dim=1))
        ...
        for ep in range(args.epochs):
            ...
            for x, oh, y in dl_tr:
                with torch.autocast("cuda", torch.float16):
                    logit = model(x.to(device), oh.to(device))
