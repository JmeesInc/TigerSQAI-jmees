...
## 2026-09-09 Astra Round 4: 実装済み、統計ゲートは不合格

ユーザー最終決定を反映: 円形FOVは生成しない、Instrument生成は既定無効、Other対象外、血液/剥離面は維持。較正はID3–30のみ、各フレームの解剖画素で正規化して平均。実528枚の最終共有基準を assets/reference_anatomy_only.yaml に保存。weighted IoUは実class0画素を無視し、実解剖領域での合成0は誤りとして残す。

公開アトラスの左右肋骨包絡から壁側胸膜10を構築し、肋間を被覆、ポートの局所開口を維持。近似形状とライセンスをdocs/round4_20260909.mdに記載。元IPVメッシュは変更していない。
