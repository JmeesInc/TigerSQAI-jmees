...
class InDomainToolPaster:
    ...
    def __call__(self, img, fine, coarse):
        ...
        for _ in range(np.random.randint(1, self.max_tools + 1)):
            # 貼り付け 1 回につき 1 度だけ数え直す。配置候補ごとに全画像を走査すると
            # 10 クラス x 8 リトライ = 80 回のフルスキャンになりデータローダが詰まる。
            # 一方 1 回きりにすると 2 本目が 1 本目の欠損を見落とし保護が緩む。
            totals = np.bincount(fine.ravel(), minlength=256)
            w3 = [c for c in w3_all if totals[c] > 0]
            ...
            for _try in range(8):     # w=3 を壊さない配置が見つかるまで試す
                ...
                # 貼り付け範囲内だけを 1 回数える (全画像スキャンを避ける)
                hit = np.bincount(sub[p_m], minlength=256)
                ok = all(hit[c] / totals[c] <= self.protect_w3 for c in w3)
                if ok:
